...
// ErrorHandler — централизованный обработчик ошибок HTTP-запросов.
// Предназначен для единообразной генерации ответа клиенту в случае ошибки
// и одновременного логирования инцидента с привязкой к контексту запроса.
//
// Основные задачи:
//   - Формирование корректного HTTP-статуса и тела ответа в формате JSON;
//   - Логирование ошибки с дополнительными полями (IP клиента, метод, путь, идентификатор запроса);
//   - Исключение дублирования кода обработки ошибок в каждом хендлере.
//
// ErrorHandler удобно вызывать в любом месте цепочки middleware или в бизнес-логике,
// где возникает ошибка, чтобы гарантировать одинаковый формат ответа и логирования.
